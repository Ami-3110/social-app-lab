...
<template>
  <main class="p-6 max-w-xl mx-auto space-y-4 ui-bg ui-text min-h-screen">
    ...
    <div v-else-if="error" class="text-red-600 text-sm">Failed to load following.</div>
...
const { data: res, pending, error, refresh } = useAsyncData<{ data: UserLite[] }>(
  () => `user:${userId.value}:following`,
  () => $apiFetch(`/users/${userId.value}/following`),
  { watch: [userId] }
)
...
const onToggleFollow = async (targetUserId: number, nextFollow: boolean) => {
  ...
  try {
    ...
    // このページの表示自体はoptimisticでOK。
    // もし「フォロー解除したら一覧から消したい」なら次の1行をONに：
    // await refresh()
  } catch (e) {
